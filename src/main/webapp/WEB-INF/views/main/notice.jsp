									...
									<tbody>
										<c:forEach items="${list}" var="board">
											<tr class="">
												...
												<td class="td_subject"><center><a href="readn?bno=${board.bno}"
													style="color: black;">${board.title}</a></center></td>
												<td class="td_name sv_use">${board.writer}</td>
												<td class="td_date"><fmt:formatDate pattern="MM - dd"
...
<fieldset id="bo_sch">
    ...
    <select name="searchType">
			...
			<option value="num"
				<c:out value="${cri.searchType == 'num'?'selected':''}"/>>글
				번호
			<option value="na"
				<c:out value="${cri.searchType == 'na'?'selected':''}"/>>글
				제목
			<option value="re"
				<c:out value="${cri.searchType == 're'?'selected':''}"/>>작성자

		</select>
    
    
    <label for="stx" class="sound_only">검색어<strong class="sound_only"> 필수</strong></label>
    <input type="text" name="keyword" value='${cri.keyword}' required id="stx" class="frm_input required" size="15" maxlength="15">
    <input type="submit" value="검색" id='searchBtn' class="btn_submit">
    </form>
</fieldset>
...
<c:choose>	 
	  <c:when test="${userid == 'admin'}">
			<center><button type="button" class="btn btn-default"
						onclick="location.href='register'">글쓰기</button></center>
	  </c:when> 
	  <c:otherwise>
	  </c:otherwise>
</c:choose>
